...
def _convert_spect_to_probability(spect,freqs):
    ...
    # probability
    prob = amplitude_spectrum / numpy.matlib.repmat(np.sum(amplitude_spectrum, 0), num_rows, 1)
...
def _variance(freqs_mat,spect_centroid,num_rows,prob):
    ...
    return np.sum((np.power(freqs_mat - numpy.matlib.repmat(spect_centroid, num_rows, 1), 2)) * prob, 0)
...
def spectral_skewness(spect, freqBins):
    ...
    spect_centroid = spectral_centroid(prob,freqs_mat)
    variance = _variance(freqs_mat,spect_centroid,num_rows,prob)
    skewness = np.sum((np.power(freqs_mat - numpy.matlib.repmat(spect_centroid, num_rows, 1), 3)) * prob, 0)
...
def spectral_kurtosis(spect, freqBins):
    ...
    spect_centroid = spectral_centroid(prob,freqs_mat)
    variance = _variance(freqs_mat,spect_centroid,num_rows,prob)
    kurtosis = np.sum((np.power(freqs_mat - numpy.matlib.repmat(spect_centroid, num_rows, 1), 4)) * prob, 0)
...
def _cepstrum_for_pitch(spect, nfft, samp_freq, min_freq, max_freq):
    ...
    exs = np.vstack((amplitude_spectrum,
                     numpy.matlib.repmat(amplitude_spectrum[-1, :], nfft, 1),
